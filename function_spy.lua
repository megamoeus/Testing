...
local function createEnv(spy)
    ...
    setmetatable(env, {
        __index = function(_, key)
            local val = _G[key]
            if val == nil and spy.settings.spynilglobals then
                spy.globals[key] = true
            end
            return wrapValue(spy, val)
        end,
...
-- if executed directly, hook input.lua
if ... == nil then
    local file = "input.lua"
    local chunk, err = loadfile(file)
    if chunk then
        local spy = FunctionSpy.new()
        local wrapped = spy:hook(chunk)
        local ok, msg = pcall(wrapped)
        if not ok then
            io.stderr:write("error running " .. file .. ": " .. tostring(msg) .. "\n")
        end
        print(spy:getReport())
    else
